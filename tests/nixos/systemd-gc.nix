...
in
{
  ...
  testScript = if official then ''
    ...
    machine.log(machine.succeed("wrapper=$(systemctl show nix-gc.service -p ExecStart | sed -n 's/.*path=\\([^; ]*\\).*/\\1/p'); stat -c '%F %s %N' \"$wrapper\"; readlink -f \"$wrapper\"; sed -n 'l' \"$wrapper\""))
    machine.log(machine.succeed("nix-maintenance-status-systemd-vm-probe --system"))
